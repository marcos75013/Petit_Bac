...
function addWord(word) {
  ...
  if (word.charAt(0).toLowerCase() === letter && !words.includes(word)) {
    ...
    // Find the category corresponding to the current letter
    let category = document.querySelector(`#categories li:nth-child(${words.length})`);

    // Insert the new word before the first element of the category's word list
    category.parentNode.insertBefore(li, category.nextSibling);
  }
}
